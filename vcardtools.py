...
def init_parser():
    parser = argparse.ArgumentParser( \
        description='Automatically fix, split, normalize, group and merge/deduplicate vCard and VCF files (even large ones).' \
    )
    parser.add_argument( \
    ...
    parser.add_argument( \
        ...
        help="Disable fixing invalid lines, and broken multilines value, and converting from vCard 2.1 to 3.0" \
    )
    parser.add_argument( \
    ...
    parser.add_argument( \
        ...
        help="Enable french tweaks (phone number '+33' converted to '0', handling of the name particule ' De ')." \
    )
    parser.add_argument( \
        '-a', '--match-attributes', dest='match_attributes', action='append', \
        default=vcardlib.OPTION_MATCH_ATTRIBUTES, \
        help="Use those attributes to match vCards. Specials attributes: 'names' is an alias for 'fn'+'n' and 'mobiles' for 'tel'+filter by phone number" \
    )
    parser.add_argument( \
        '-t', '--match-ratio', dest='match_ratio', type=int, default=100, \
        help="The ratio score to match the names (see fuzzywuzzy documentation). Default is: 100 (safe)." \
    )
    parser.add_argument( \
    ...
    parser.add_argument( \
        '-1', '--no-match-same-first-letter', dest='no_match_same_first_letter', action='store_true', \
    ...
    parser.add_argument( \
        ...
        help="Use the startswith comparizon (using --match-max-distance) when doing approximate matching" \
    )
    parser.add_argument( \
        '--move-name-extra-info-to-note', dest='move_name_parentheses_or_braces_to_note', action='store_true', \
...
def main():
    ...
    try:
        ...
        if args.match_attributes and isinstance(args.match_attributes, list):
            vcardlib.OPTION_MATCH_ATTRIBUTES = args.match_attributes
        ...
        if args.match_max_distance and isinstance(args.match_max_distance, int):
            vcardlib.OPTION_MATCH_APPROX_MAX_DISTANCE = range(-args.match_max_distance, args.match_max_distance)
        ...
        if not vcardlib.OPTION_NO_MATCH_APPROX:
            logging.info("\tMATCH_APPROX_SAME_FIRST_LETTER: %s",vcardlib.OPTION_MATCH_APPROX_SAME_FIRST_LETTER)
            ...
            logging.info("\tMATCH_APPROX_MAX_DISTANCE: %s",vcardlib.OPTION_MATCH_APPROX_MAX_DISTANCE)
            ...
            for g_name, g_list in sorted(vcards_grouped.items()):
                if len(g_list) > 1:
                    ...
                    # group
                        ...
                        for key in g_list:
                            logging.debug("\t\t%s", key)
                            write_vcard_to_file(vcards[key], d_path + '/' + key.replace('/', '-') + '.vcard')
